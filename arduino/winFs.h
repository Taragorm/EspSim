...
//    File(FileImplPtr p = FileImplPtr(), FS *baseFS = nullptr) : _p(p), _fakeDir(nullptr), _baseFS(baseFS) { }
    WinFile(std::FILE* fp = nullptr)
    : _file(fp)
    {}
    //-------------------------------------------------

    WinFile(WinFile& other)
    : _file(other._file)
    {
       other._file = nullptr; // copy keeps the handle
    }
    //-------------------------------------------------
...
//    virtual int availableForWrite() override;
    ...
    // Stream methods:
    virtual int available() override { return 1; } //todo
...
//    Dir(DirImplPtr impl = DirImplPtr(), FS *baseFS = nullptr): _impl(impl), _baseFS(baseFS) { }
    ...
    WinFile openFile(const char* mode);
...
public:
    ...
    WinFile open(const char* path, const char* mode);
    WinFile open(const String& path, const char* mode);
